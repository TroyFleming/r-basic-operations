...
```{r}
# Use pound sign similar to python
# Subtle differences between the two, so be aware
```
...
```{r}
# It's recommened to use <- to assign variables, as using
# the = sign will cause conflicts/errors in code
# Shortcut key: alt + -
x <- 10
x
```
...
```{r}
# Assgining variable strings to another variable requires the paste() function
# paste(variable_1, variable_2) -- will automatically add space between variables
first_name <- "Joe"
last_name <- "Smith"
...
```{r}
# as.character() essentially coverts what's entered to a string
x <- as.character(12)
x
```
...
```{r}
# print() function is essentially a straight way to print text/numbers
...
```{r}
# In example, %s represents a string, %d represents an integer value
sprintf("%s ate %d cookies.", "John", 21)
```

```{r}
# In example, %f represents a float, %e double represents precision value
...
```{r}
# Pulls from a string using start position and ending position of characters
# Note that it does not start at 0, but at 1. First character in string is 1.
...
```{r}
# Format: target string to be replaced, replacement string, source string
...
```{r}
# Booleans must be ALL CAPS IN R
x <- TRUE
y <- FALSE
```
...
1.  Create two strings with your first and last name, respectively. Then concatenate them, and print the new value as "My name is \<your newly created concatenated variable will go here\>"
    ...
    t_first_name <- "Troy"
    t_last_name <- "Fleming"
    t_full_name <- paste(t_first_name, t_last_name)
    sprintf("My name is %s", t_full_name)
    ```
